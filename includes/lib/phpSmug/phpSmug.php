...
class SME_phpSmug {
	...
	{
		$args = SME_phpSmug::processArgs( func_get_args() );
		...
		$this->req->setHeader( array( 'User-Agent' => "{$this->AppName} using SME_phpSmug/" . SME_phpSmug::$version, 'Content-Type' => 'application/x-www-form-urlencoded' ) );
    }
	
	/**
	 * General debug function used for testing and development of SME_phpSmug.
	 *
	...
	public function enableCache()
	{
		...
		$this->cache_table  = ( array_key_exists( 'table', $args ) ) ? $args['table'] : 'SME_phpSmug_cache';
        ...
        if ( $this->cacheType == 'db' ) {
			...
			$options = array( 'comment' => 'SME_phpSmug cache', 'charset' => 'utf8', 'collate' => 'utf8_unicode_ci' );
    ...
    private function getCached( $request )
	{
		...
		if ( $this->cacheType == 'db' ) {
			...
			if ( PEAR::isError( $result ) ) {
				throw new SME_phpSmugException( $result );
			}
			if ( !empty( $result ) ) {
                return $result;
            }
	...
	private function request( $command, $args = array() )
	{
		if ( ( strpos( $command, 'login.with' ) || strpos( $command, 'Token' ) ) || ( $this->oauth_signature_method == 'PLAINTEXT' ) || $this->secure ) {
			...
			if ( ( isset( $this->SessionID ) && is_null( $this->SessionID ) ) && ( !strpos( $command, 'login.anonymously' ) ) && !$this->OAuthSecret ) {
				throw new SME_phpSmugException( 'Not authenticated. No Session ID or OAuth Token.  Please login or provide an OAuth token.' );
			}
		}
		...
		$values = array_map( array( 'SME_phpSmug', 'urlencodeRFC3986' ), array_values( $args ) );
		...
		if ( $this->parsed_response['stat'] == 'fail' ) {
			...
			throw new SME_phpSmugException( "SmugMug API Error for method {$command}: {$this->error_msg}", $this->error_code );
		} else {
	...
	public function images_upload()
	{
		...
		if ( !array_key_exists( 'File', $args ) ) {
			throw new SME_phpSmugException( 'No upload file specified.' );
		}
		...
		if ( is_file( $args['File'] ) ) {
			...
			throw new SME_phpSmugException( "File doesn't exist: {$args['File']}" );
		}
		...
		$upload_req->setHeader( array( 'User-Agent' => "{$this->AppName} using SME_phpSmug/" . SME_phpSmug::$version,
		...
		if ( $this->parsed_response['stat'] == 'fail' ) {
            ...
            $this->error_msg = $this->parsed_response['message'];
			$this->parsed_response = FALSE;
			throw new SME_phpSmugException( "SmugMug API Error for method image_upload: {$this->error_msg}", $this->error_code );
		} else {
			$this->error_code = FALSE;
            $this->error_msg = FALSE;
		}
	...
	public function __call( $method, $arguments )
	{
		...
		$args = SME_phpSmug::processArgs( $arguments );
	 ...
	 public function authorize()
	{
	 ...
	 private function generate_signature( $apicall, $apiargs = NULL, $url = NULL )
	 {
		...
		if ( $this->oauth_signature_method == 'PLAINTEXT' ) {
			return SME_phpSmug::urlencodeRFC3986( $this->OAuthSecret ).'&'.SME_phpSmug::urlencodeRFC3986( $this->oauth_token_secret );
		} else {
		    ...
		    $values = array_map( array( 'SME_phpSmug', 'urlencodeRFC3986' ), array_values( $params ) );
			...
			$base_string = $method . '&' . SME_phpSmug::urlencodeRFC3986( $endpoint ) . '&' .  SME_phpSmug::urlencodeRFC3986( $string );
...
interface SME_phpSmugRequestProcessor
{
...
class httpRequest
{
    ...
    */
	...
	public function __construct( $url = NULL, $method = 'POST', $timeout = 30 )
	{
		...
		$this->setHeader( array( 'User-Agent' => "Unknown application using SME_phpSmug/" . SME_phpSmug::$version ) );
		...
		if ( function_exists( 'curl_init' ) && ( $this->config['adapter'] == 'curl' )
			...
			$this->processor = new SME_phpSmugCurlRequestProcessor;
		}
		else {
			$this->processor = new SME_phpSmugSocketRequestProcessor;
		}
	}
...
class SME_phpSmugCurlRequestProcessor implements SME_phpSmugRequestProcessor
{
	private $response_body = '';
	private $response_headers = '';
	private $executed = FALSE;
...
class SME_phpSmugSocketRequestProcessor implements SME_phpSmugRequestProcessor
{
	private $response_body = '';
	private $response_headers = '';
	private $executed = FALSE;
